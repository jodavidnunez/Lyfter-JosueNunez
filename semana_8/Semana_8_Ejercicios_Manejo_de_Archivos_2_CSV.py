...
def build_games_csv():
    ...
    try:
        write_csv_file(output_file, games_data_dicts_list, csv_headers)
        print(f'-I-(build_games_csv) CSV with the requested information recorded in the path : {output_file}')
    except IOError:
        print(f'-E-(build_games_csv) An error showed up while trying to write the output csv file \'{output_file}\''
              f'-I-(build_games_csv) Please verify the output directory \'{output_dir}\' exists,' 
...
try:
    ...
    print(f'\n-E-(build_games_csv): Manual interruption by user.')